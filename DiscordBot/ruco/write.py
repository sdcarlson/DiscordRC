'''
The write module provides all the functions necessary to write any permissions
for any type of Discord server.
'''
...
class Write(commands.Cog):
    '''
    The Write class translates a JSON server config into
    a set of Discord API calls.
    
    The `rules_ch_pos` and `updates_ch_pos` are integers obtained from the
    position attribute of the Discord's rule Channel and updates Channel object.
    Each attribute represents the respective channel's index,
    where the index represents the order in which the channel appears in the
    Discord server UI. Note that the text channel index does not consider
    channel types such as forum channels and those in voice-based channels.

    The `rules_ch_idcs` and `updates_ch_idcs` are indices that are generated by
    the functions in the Write class, where the first and second integer
    represents the index of the category and channel respectively.
    '''
    ...
    async def _write_role(self, ctx, role_config):
        '''
        Writes role permissions given a role_config.

        `role_config`: If the role is not in the Discord server,
            the role should have an id attribute of `None`,
            for which the `_write_role` function will create the role
            and update the role_config with the respective id.
        Returns role_config with the updated id.
        '''
    ...
    def _set_ch_creator(self, ctx, cat, ch_id, ch_name, ch_type):
        '''
        Sets the channel creator, which differs depending on the channel type.

        `ch_type`: The enum representation of the channel type imported from
    ...
    def _set_ch_perms_for_role(self, overwrite, ch_perms_for_role, ch_type):
        '''
        Sets the discord.PermissionsOverwrite() object based on the
        `ch_perms_for_role` attribute. The object is applied to the role in some
        later function.

        `overwrite` (obj): A discord.PermissionOverwrite() object,
            specifying which permissions to change.

        `ch_perms_for_role` (dict): The channel role permissions dictionary with
            key-value pairs of permission name and permission boolean value.
        '''
    ...
    async def _move_ch(self, ctx, ch, ch_idcs):
        '''
        Move the channel to the position specified by `ch_idcs`.
        In the current implementation, only the member variables `rules_ch_idcs`
        and `updates_ch_idcs` are passed into the function.

        `ch_idcs` (int, int): The first index specifies the category index,
            and the second index specifies the channel index.
        '''
    ...
    async def _write_ch(self, ctx, ch_config, role_ids, cat_id):
        ...
        or `voice_based_channel` list in the JSON config).

        `cat_id` (int): The id of the category that the channel belongs to.
        `ch_config` (dict): The channel dictionary specifying the 
            respective properties and permissions.
        Returns a tuple of the discord.Channel object and the ch_config with
        updated ids.
        '''
        cat = None
        if cat_id:
    ...
    def _set_cat_perms_for_role(self, overwrite, role, cat_perms_for_role):
        '''
        Similar to _set_ch_perms_for_role, but for category role permissions.
        Sets the discord.PermissionsOverwrite() object based on the
        `cat_perms_for_role` attribute. The object is applied to the role in
        some later function.

        `overwrite` (obj): A discord.PermissionOverwrite() object,
            specifying which permissions to change.

        `ch_perms_for_role` (dict): The category role permissions dictionary
            with key-value pairs of permission name and
    ...
    async def _write_cat(self, ctx, cat_config, role_ids):
        '''
        Create a category with properties and permissions specified by
        a category dictionary (which is an element of a category list keyed by
        'categories' in the top level of the the JSON config).

        `cat_config` (dict): The category dictionary specifying the 
            respective properties and permissions.
        Returns a cat_config with updated ids.
        '''
    ...
    async def _create_required_community_chs(self, ctx, config):
        ...
        Returns the discord.Channel objects for rules and updates channels.
        '''
        pos_update = 0
        rules_ch = None
        updates_ch = None
    ...
    async def write_server(self, ctx, config):
        ...
        Returns the config with updated ids.
        '''
        self.config = config
